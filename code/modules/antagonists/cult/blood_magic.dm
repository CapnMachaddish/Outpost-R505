		...
		if(ishuman(target))
			...
			if(iscultist(H))
				if(H.stat == DEAD)
					...
					return
				if(H.blood_volume < H.blood_volume_threshold(BLOOD_VOLUME_SAFE))
					var/restore_blood = H.blood_volume_threshold(BLOOD_VOLUME_SAFE) - H.blood_volume
					if(uses*2 < restore_blood)
						...
						H.blood_volume = H.blood_volume_threshold(BLOOD_VOLUME_SAFE)
