...
/obj/item/clothing/sextoy/vibrator/process(delta_time)
	if(toy_on == TRUE)
		if(vibration_mode == "low" && arousal_comp.arousalloss < 30)
			arousal_comp.adjustArousalLoss(0.7 * delta_time)
		if(vibration_mode == "medium" && arousal_comp.arousalloss < 40)
			arousal_comp.adjustArousalLoss(1 * delta_time)
		if(vibration_mode == "hard" && arousal_comp.arousalloss < 70) //MERCY, MERCY!
			arousal_comp.adjustArousalLoss(1.5 * delta_time)
	if(toy_on == FALSE && arousal_comp.arousalloss < 30)
