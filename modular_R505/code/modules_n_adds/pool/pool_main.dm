/turf/open/pool
	icon = 'modular_R505/icons/turf/pool.dmi'
	name = "poolwater"
			...
			if(iscarbon(AM))		//FUN TIME!
				...
				if(filled)
					if (H.wear_mask && H.wear_mask.flags_cover & MASKCOVERSMOUTH)
						...
						playsound(src, 'modular_R505/sound/pool/splash.ogg', 60, TRUE, 1)
						H.Knockdown(20)
						return
					else
						H.Knockdown(60)
					...
					if(prob(75))
						...
						playsound(src, 'sound/effects/woodhit.ogg', 60, TRUE, 1)
						playsound(src, 'modular_R505/sound/pool/crack.ogg', 100, TRUE)
				else
					H.visible_message("<span class='danger'>[H] falls in the drained pool, but had a helmet!</span>",
										"<span class='userdanger'>You fall in the drained pool, but you had a helmet!</span>")
					H.Knockdown(40)
					playsound(src, 'sound/effects/woodhit.ogg', 60, TRUE, 1)
		else if(filled)
...
/turf/open/pool/attack_hand(mob/living/user)
	...
	if((user.loc != src) && !user.IsStun() && !user.IsKnockdown() && !user.incapacitated() && Adjacent(user) && HAS_TRAIT(user, TRAIT_SWIMMING) && filled && (next_splash < world.time))
		playsound(src, 'modular_R505/sound/pool/watersplash.ogg', 8, TRUE, 1)
