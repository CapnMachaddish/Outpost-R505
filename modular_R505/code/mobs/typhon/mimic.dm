...
/mob/living/simple_animal/hostile/typhon/mimic/ShiftClickOn(atom/movable/A)
	if(playerTfTime <= world.time && stat == CONSCIOUS)
		...
		if(istype(A) && allowed(A))
			stealthed = TRUE
			SEND_SOUND(src, sound('modular_R505/sound/creatures/mimic/mimic_transform.ogg',volume=50))
			name = A.name
			icon = A.icon
...
/datum/round_event/mimic_infestation/announce(fake)
	priority_announce("Unidentified lifesigns detected aboard [station_name()]. Secure any exterior access, including ducting and ventilation.", "Lifesign Alert")
